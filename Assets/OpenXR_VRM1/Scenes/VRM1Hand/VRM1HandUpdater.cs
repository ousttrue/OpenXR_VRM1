...
{
    static Dictionary<(int, XrHandJointEXT), HumanBodyBones> JointToBone = new Dictionary<(int, XrHandJointEXT), HumanBodyBones>
        {
            // left
    ...
    {
        foreach (var (k, v) in JointToBone)
        {
            if (k.Item1 != leftRight)
            {
                continue;
            }
            var joint = joints[(int)k.Item2];

            var t = vrm_.Runtime.ControlRig.GetBoneTransform(v);
            if (t != null)
            {
                if (k.Item2 == XrHandJointEXT.XR_HAND_JOINT_WRIST_EXT)
                {
                    if (vrm_.TryGetBoneTransform(v, out var handTransform))
                    {
